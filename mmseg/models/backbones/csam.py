...
class ConvolutionalSelfAttention(nn.Module):
    def __init__(
        ...
        self.bn_flag = (
            hasattr(self.approach_args, "ADD_BATCH_NORM")
            and self.approach_args.ADD_BATCH_NORM
        )
        self.ln_flag = (
            hasattr(self.approach_args, "ADD_LAYER_NORM")
            and self.approach_args.ADD_LAYER_NORM
        )
        if self.bn_flag:
        ...
        if self.use_residual_connection and output_shrunk:
            self.upsampler = torch.nn.Upsample(
                size=(self.input_H, self.input_W), mode="bilinear"
            )
        else:
        ...
        if self.approach_name in {
            ...
            self.padding_mask = self.padding_mask.reshape(
                [1, 1, self.spatial_H, self.spatial_W, 1]
            )
    ...
    def compute_padding(self, padding_type):
        if padding_type.lower() == "valid" or (
            "self_attention" in self.approach_name
            and "local_window" not in self.approach_name
        ):
            padding_tuple = (0, 0, 0, 0)
        elif padding_type.lower() == "same":
            padding_x = int(
                (self.stride * (self.input_W - 1) - self.input_W + self.filter_K) / 2
            )
            padding_y = int(
                (self.stride * (self.input_H - 1) - self.input_H + self.filter_K) / 2
            )
        ...
        input_padder = nn.ConstantPad2d(padding_tuple, 0.0)
    ...
    def setup_approach(self):
        ...
        if (
            "self_attention" not in self.approach_name
            and self.approach_args.get("pos_emb_dim", 0) > 0
        ):
            self.X_encoding_dim += self.pos_emb_dim
        if self.approach_name == "One":
            self.global_transform = nn.Linear(
                self.X_encoding_dim, self.filter_K * self.filter_K
            )
            ...
            self.indices = np.array(
                [
                    (i, j)
                    for i in range(self.convs_height)
                    for j in range(self.convs_width)
                ]
            )
            ...
            if self.approach_name in {"Three_unmasked_cls", "Three_unmasked_cls_proj"}:
                self.cls = nn.Parameter(
                    torch.rand(1, self.spatial_C, requires_grad=True)
                )
                if self.approach_name == "Three_unmasked_cls_proj":
                    self.conv_proj = nn.Linear(
                        self.spatial_C, self.spatial_C * self.spatial_C
                    )
            if self.approach_args.FORGET_GATE_TYPE == "sigmoid":
                self.forget_gate_nonlinearity = self.apply_local_sigmoid
            elif self.approach_args.FORGET_GATE_TYPE == "softmax":
            ...
            if self.approach_name in [
                ...
                elem_scales_init = torch.rand(
                    self.spatial_H, self.spatial_W, 1, requires_grad=True
                )
                with torch.no_grad():
                    elem_scales_init = elem_scales_init.view(
                        self.spatial_H * self.spatial_W, 1
                    )
        ...
        elif self.approach_name in {"gaussian_blur", "box_filter"}:
            pass
        else:
    ...
    def compute_input_mask(self):
        ...
        cell_heights = self.compute_cell_lengths(
            self.convs_height, unit_stride_convs_height
        )
        cell_widths = self.compute_cell_lengths(
            self.convs_width, unit_stride_convs_width
        )

        cell_row_starts = torch.cumsum(
            torch.cat((torch.zeros(1, dtype=torch.int), cell_heights[:-1])), dim=0
        )
        cell_col_starts = torch.cumsum(
            torch.cat((torch.zeros(1, dtype=torch.int), cell_widths[:-1])), dim=0
        )
        ...
        self.local_indices = torch.zeros(
            (self.num_convs, self.filter_K * self.filter_K)
        )
        ...
        for i_idx, i in enumerate(cell_row_starts):
            for j_idx, j in enumerate(cell_col_starts):
                offset_i = i + (
                    random.randrange(cell_heights[i_idx])
                    if self.apply_stochastic_stride
                    else 0
                )
                offset_j = j + (
                    random.randrange(cell_widths[j_idx])
                    if self.apply_stochastic_stride
                    else 0
                )
                ...
                input_mask[
                    conv_idx,
                    offset_i : offset_i + self.filter_K,
                    offset_j : offset_j + self.filter_K,
                ] = 1.0
                self.local_indices[conv_idx] = (
                    input_mask[conv_idx].reshape(-1).nonzero().sort()[0].reshape(-1)
                )
    ...
    def split_input(self, batch, mask_X_g=True):
        ...
        if mask_X_g:
            ...
            X_g = X_g.reshape(
                self.num_convs, -1, self.spatial_H * self.spatial_W, self.X_encoding_dim
            )  # [F,B,HW,E]
            X_gi = torch.argsort((X_g.sum(-1) != 0).type(torch.float32), dim=-1)[
                :, :, self.filter_size :
            ].sort(dim=-1)[
                0
            ]  # [F,B,(HW-K^2)]
            ...
            X_g_flat_ = X_g_flat[
                torch.arange(X_g_flat.shape[0]).unsqueeze(-1), X_gi_flat
            ]  # [FB,(HW-K^2),E]
            X_g_ = X_g_flat_.reshape(
                self.num_convs, -1, X_g_flat_.shape[1], X_g_flat_.shape[2]
            )
        else:
            X_g_ = batch_r.reshape(
                1, -1, self.spatial_H * self.spatial_W, self.X_encoding_dim
            )  # [1,B,HW,E]
        X_l = (batch_r * self.local_mask).reshape(  # [F,B,H,W,E]
            self.num_convs,
            -1,
            self.spatial_H * self.spatial_W,
            self.X_encoding_dim,  # [F,B,HW,E]
        )
        X_li = torch.argsort((X_l.sum(-1) != 0).type(torch.float32), dim=-1)[
            :, :, -self.filter_size :
        ].sort(dim=-1)[
            0
        ]  # [F,B,K^2]
        ...
        X_l_flat_ = X_l_flat[
            torch.arange(X_l_flat.shape[0]).unsqueeze(-1), X_li_flat
        ]  # [FB,K^2,E]
        X_l_ = X_l_flat_.reshape(
            self.num_convs, -1, self.filter_size, self.X_encoding_dim
        )  # [F,B,K^2,E]
        return X_l_, X_g_
    ...
    def efficient_approach1(self, batch):
        ...
        global_mask = (
            1 - self.padding_mask.reshape(1, -1)
        ) - self.local_mask  # [Nc,HW]
        convolutions = torch.einsum(
            ...
            2, 1
        )  # [B,K^2,HW]x[HW,Nc] -> [B,K^2,Nc] -> [B,Nc,K^2]
        local_idxs_onehot = F.one_hot(
            self.local_indices.type(torch.int64)
        )  # OneHot([Nc,HW]) -> [Nc,K^2,HW]
        Xl = torch.einsum(
            "ijk,lmj->ilmk", batch_flat, local_idxs_onehot.type(torch.float)
        )  # [B,HW,C],[Nc,K^2,HW] -> [B,Nc,K^2,C]
        result = torch.einsum(
            "abcd,abc->abd", Xl, convolutions
        )  # [B,Nc,K^2,C]x[B,Nc,K^2] -> [B,Nc,C]
    ...
    def maybe_add_positional_encodings(self, batch):
        if self.positional_encodings is not None:
            ...
            if (
                "self_attention" in self.approach_name
                or self.approach_args.get("pos_emb_dim", 0) < 0
            ):
                return batch + positional_encodings
            return torch.cat(
                (batch, positional_encodings), dim=-1
            )  # [[B,H,W,C];[B,H,W,P]] -> [B,H,W,C+P]
        return batch
    ...
    def efficient_approach2(self, batch):
        ...
        score = torch.bmm(queries, keys.transpose(1, 2)) / math.sqrt(
            queries.shape[-1]
        ...
        exp_sum = torch.matmul(
            exp_sim, global_mask.transpose(1, 0)
        )  # [B,HW,HW]x[HW,Nc] -> [B,HW,Nc]
        # Invert denominator to obtain \frac{1}{ Σ_j^{HW-K^2} e^{ matmul(Q, [K.T]_j) } }
        # & add epsilon for numerical stability
        inverted_sum = 1.0 / (exp_sum + 6.1e-5)  # [B,HW,Nc]
        # Softly transform denominator from all HW to just K^2 by masking out all sums not in X_l
        masked_denominator = inverted_sum * self.local_mask.transpose(1, 0).unsqueeze(
            0
        )  # [B,HW,Nc]x([Nc,Hw] -> [HW,Nc] -> [1,HW,Nc]) -> [B,HW,Nc]
        # Compute numerator x value: V_i x e^{ matmul(Q, [K.T]_i) }
        g_sim = (
            values.transpose(2, 1) * exp_sim
        )  # ([B,HW,1] -> [B,1,HW])x[B,HW,HW] -> [B,HW,HW]
        # Compute numerator sum: Σ_i^{HW-K^2} V_i x e^{ matmul(Q, [K.T]_i) }
        g_sum = torch.matmul(
            g_sim, global_mask.transpose(1, 0)
        )  # [B,HW,HW]x([B,Nc,HW] -> [B,HW,Nc]) -> [B,HW,Nc]
        ...
        result = torch.bmm(batch_flat.transpose(2, 1), extended_filter).transpose(
            2, 1
        )  # ([B,HW,C] -> [B,C,HW])x[B,HW,Nc] -> [B,C,Nc] -> [B,Nc,C]
    ...
    def approach3_v2(self, batch):
        ...
        filter_vals = (filter_vecs * X).sum(
            -1, keepdim=True
        )  # [B,HW,C] x [B,HW,C] -> [B,HW,1]
        ...
        output = self.forget_gate_nonlinearity(
            filter_raw=filter_vals, pooling_features=local_values
        )
        # pdb.set_trace()
    ...
    def approach3_v2_base(self, batch):
        ...
        if not self.approach_args.SIMILARITY_METRIC == "avg_pool":
            ...
            attn = self.masked_softmax(  # [B,HW,HW]
                scores,
                mask=valid_elements.flatten()
                .unsqueeze(0)
                .unsqueeze(0),  # Mask out padding indices [1, 1, HW]
                dim=-1,
                epsilon=1e-5,
            )  # [B,HW,HW]
            ...
            filter_vecs = (
                values.sum(1, keepdim=True)
                * valid_elements.flatten().unsqueeze(0).unsqueeze(-1)
            ) / valid_elements.sum()
        filter_vals = (filter_vecs * X).sum(
            -1, keepdim=True
        )  # [B,HW,C] x [B,HW,C] -> [B,HW,1]
        ...
        # output = torch.matmul(weighted_X.transpose(2, 1), local_mask).transpose(2, 1)                                 # [B,C,HW] x [HW,Nc] -> [B,C,Nc]
        output = self.forget_gate_nonlinearity(
            filter_raw=filter_vals, pooling_features=X
        )
        if (
            hasattr(self.approach_args, "OUTPUT_PROJECTION")
            and self.approach_args.OUTPUT_PROJECTION
        ):
    ...
    def approach3_v2_cls(self, batch):
        ...
        if self.approach_args.SIMILARITY_METRIC == "cosine_similarity":
            scores = torch.matmul(X_normed, cls_normed.transpose(1, 0)).transpose(
                2, 1
            )  # [B,HW,C] x ([1,HW] -> [HW,1]) -> [B,HW,1] -> [B,1,HW]
        elif self.approach_args.SIMILARITY_METRIC == "dot_product":
            scores = torch.matmul(X, self.cls.transpose(1, 0)).transpose(
                2, 1
            )  # [B,HW,C] x ([1,HW] -> [HW,1]) -> [B,HW,1] -> [B,1,HW]
    ...
    def approach3_v2_cls_proj(self, batch):
        ...
        filter_vals = (filter_vec * X).sum(
            -1, keepdim=True
        )  # [B,1,C] x [B,HW,C] -> [B,HW,1]
        projection_matrices = self.conv_proj(filter_vec.squeeze(1)).reshape(
            -1, self.spatial_C, self.spatial_C
        )  # f([B,1,C] -> [B,C]) -> [B,CC] -> [B,C,C]
        X_projected = torch.bmm(X, projection_matrices)  # [B,HW,C] x [B,C,C]
        output = self.forget_gate_nonlinearity(
            filter_raw=filter_vals, pooling_features=X_projected
        )  # [B,Nc,C]
        return output.reshape(
            batch_size, self.convs_height, self.convs_width, self.spatial_C
        )
    ...
    def gaussian_blur(self, batch):
        ...
        # pdb.set_trace()
        output = torchvision.transforms.functional.gaussian_blur(
            X, kernel_size=self.filter_K
        )
    ...
    def apply_local_softmax(self, filter_raw, pooling_features):
        ...
        filter_exp = torch.exp(
            filter_raw - filter_raw.max(dim=1, keepdim=True)[0]
        ).squeeze(
            -1
        )  # [B,HW]
        filter_exp_sum = torch.matmul(filter_exp, local_mask)  # [B,Nc]
        filter_exp_sum_inv = 1.0 / (filter_exp_sum + 6.1e-5)  # [B,Nc]
        exp_weighted_features = pooling_features * filter_exp.unsqueeze(
            -1
        )  # [B,HW,C] x [B,HW,1] -> [B,HW,C]
        exp_weighted_features_sum = torch.matmul(
            exp_weighted_features.transpose(2, 1), local_mask
        ).transpose(
            2, 1
        )  # ([B,HW,C] -> [B,C,HW]) x [HW,Nc] -> [B,C,Nc] -> [B,Nc,C]
        weighted_features = exp_weighted_features_sum * filter_exp_sum_inv.unsqueeze(
            -1
        )  # [B,Nc,C] x ([B,Nc] -> [B,Nc,1]) -> [B,Nc,C]
        return weighted_features

    def apply_local_sigmoid(self, filter_raw, pooling_features):
        local_mask = self.local_mask.flatten(1).transpose(1, 0)
        weighted_X = (
            F.sigmoid(filter_raw) * pooling_features
        )  # [B,HW,C] x [B,HW,1] -> [B,HW,C]
        output = torch.matmul(weighted_X.transpose(2, 1), local_mask).transpose(
            2, 1
        )  # [B,C,HW] x [HW,Nc] -> [B,C,Nc] -> [B,Nc,C]
        return output
    ...
    def approach3(self, batch):
        if self.approach_name == "Three_unmasked":
            local_mask = (
                ...
                .cuda()
                .reshape(self.num_convs, 1, self.spatial_H, self.spatial_W, 1)
            )
        else:
            local_mask = self.local_mask
        global_mask = (
            (1 - self.padding_mask - local_mask)
            .flatten(start_dim=1, end_dim=-1)
            .reshape(self.convs_height, self.convs_width, -1)
        )  # [Nc,1,H,W,1]
        X = self.maybe_add_positional_encodings(batch)  # [B,H,W,E]
        batch_size, H, W, _ = X.shape
        ...
        # pdb.set_trace()
        output = torch.clone(self.undo_padding(batch.permute(0, 3, 1, 2))).permute(
            0, 2, 3, 1
        )
        ...
        for i, j in indices:
            ...
            X_l = (
                X[:, i : i + self.filter_K, j : j + self.filter_K] / denom
            )  # [B,K,K,C]
            ...
            raw_compatibilities = raw_compatibilities.view(
                -1, H * W, self.filter_size
            )  # [B,HW,K^2]
            ...
            W_g = torch.bmm(
                compatabilities.transpose(2, 1), X_g_vectors
            )  # ([B,HW,K^2] -> [B,K^2,HW]) x [B,HW,C] -> [B,K^2,C]
            X_l_flat_spatial = X_l[:, :, :, : self.spatial_C].reshape(
                -1, self.filter_size, self.spatial_C
            )  # [B,K^2,C]
            forget_gate = self.forget_gate_nonlinearity(
                torch.sum(W_g * X_l_flat_spatial, dim=-1, keepdim=True)
            )  # [B,K^2,1]
            output[:, i, j] = (forget_gate * X_l_flat_spatial).sum(
                dim=1
            )  # [B,K^2,1] x [B,K^2,C] -> [B,K^2,C] -> [B,C]
        return output
    ...
    def approach4(self, batch):
        ...
        W_g = elem_mul.sum(dim=2).unsqueeze(
            -1
        )  # [F,B,HW,K^2] -> [F,B,K^2] -> [F,B,K^2,1]
        X_l = X_l[:, :, :, : self.spatial_C]  # [F,B,K^2,E] -> [F,B,K^2,C]
        convolved_X = (
            (W_g * X_l).sum(dim=2).permute(1, 0, 2)
        )  # [F,B,K^2,1] x [F,B,K^2,C] -> [F,B,K^2,C] -> [F,B,C] -> [B,F,C]
    ...
    def approach4_mem_efficient(self, batch):
        global_mask = (
            (1 - self.padding_mask - self.local_mask)
            .flatten(start_dim=1, end_dim=-1)  # [Nc,1,H,W,1]
            .reshape(self.convs_height, self.convs_width, -1)  # [Nc, HW]
        )  # [F,F,HW]
        X = self.maybe_add_positional_encodings(batch)  # [B,H,W,E]
        batch_size, H, W, _ = X.shape
        ...
        output = torch.zeros(
            batch_size, convs_height, convs_width, self.spatial_C, dtype=torch.float
        ).cuda()  # [B,F,F,C]
        for i in range(0, convs_height, self.stride):
            for j in range(0, convs_width, self.stride):
                queries = all_queries[
                    :, i : i + self.filter_K, j : j + self.filter_K
                ]  # [B,K,K,E]
                raw_compatibilities = torch.einsum(
                    "bhwe,bkje->bhwkj", keys, queries
                )  # [B,H,W,K,K]
                raw_compatibilities[
                    :, i : i + self.filter_K, j : j + self.filter_K, :, :
                ] = 0
                ...
                elem_mul = (
                    values * compatabilities
                )  # [B,HW,1] x [B,HW,K^2] -> [B,HW,K^2]
                W_g = (
                    elem_mul.sum(dim=1)
                    .view(-1, self.filter_K, self.filter_K)
                    .unsqueeze(-1)
                )  # [B,HW,K^2] -> [B,K^2] -> [B,K,K] -> [B,K,K,1]
                X_l = X[
                    :, i : i + self.filter_K, j : j + self.filter_K, : self.spatial_C
                ]  # [B,K,K,C]
                output[:, i, j] = (W_g * X_l).sum(
                    dim=(1, 2)
                )  # [B,K,K,1] x [B,K,K,C] -> [B,K,K,C] -> [B,C]
        return output
    ...
    def approach5(self, batch):
        ...
        local_nands = nand_indicator * self.local_mask.transpose(1, 0).unsqueeze(
            0
        )  # [B,HW,Nc]
        grouped_localities = torch.bmm(
            keys.permute(0, 2, 1), local_nands
        )  # [B,C,HW]x[B,HW,F]
        output = grouped_localities.permute(0, 2, 1).reshape(
            -1, self.convs_height, self.convs_width, self.spatial_C
        )  # [B,C,F] -> [B,F,C] -> [B,F,F,]
        return output
    ...
    def kl_div_self_attention(self, batch):
        ...
        div = F.kl_div(probs, targets, reduction="none", log_target=True).mean(
            -1, keepdim=True
        )  # [B,HW]
    ...
    def local_window_self_attention(self, batch, include_Xl_in_Xg=False):
        ...
        scaled_local = torch.matmul(
            scaled_batch.permute(0, 2, 1), self.local_mask.transpose(1, 0)
        ).permute(
            0, 2, 1
        )  # ([B,HW,C] -> [B,C,HW]) x ([Nc,HW] -> [HW,Nc]) -> [B,C,Nc] -> [B,Nc,C]
        ...
        score = torch.bmm(local_queries, global_keys.transpose(1, 2)) / math.sqrt(
            local_queries.shape[-1]
        )  # [B,Nc,C] x ([B,HW,C] -> [B,C,HW]) -> [B,Nc,HW]
        attn = self.masked_softmax(
            vec=score, mask=global_mask.unsqueeze(0), dim=-1
        )  # [B,Nc,HW]
    ...
    def pytorch_self_attention(self, batch):
        batch_pos = (
            self.maybe_add_positional_encodings(batch).flatten(1, 2).transpose(1, 0)
        )
        ...
        attn_output, attn_output_weights = F.multi_head_attention_forward(
            ...
            1,  # q, k, v, dim, heads
            None,
            in_proj_bias,  # in_proj and in_proj bias
            None,
            None,
            False,  # bias_k, bias_v, add_zero_attn
            0,
            torch.eye(self.spatial_C).cuda(),
            torch.zeros(
                self.spatial_C
            ).cuda(),  # dropout, out proj weight, out proj bias
            training=True,
            key_padding_mask=None,
            need_weights=False,
            attn_mask=None,
            use_separate_proj_weight=True,
            q_proj_weight=self.query_transform._parameters["weight"],
            k_proj_weight=self.key_transform._parameters["weight"],
            v_proj_weight=self.value_transform._parameters["weight"],
        )

        return attn_output.transpose(1, 0).reshape(
            -1, self.input_H, self.input_W, self.spatial_C
        )
    ...
    def background_self_attention(self, batch):
        ...
        scaled_batch = (batch_pos * self.elem_scales).flatten(
            1, 2
        )  # [B,H,W,C] x [B,H,W,C] -> [B,HW,C]
        scaled_global = torch.matmul(
            scaled_batch.permute(0, 2, 1), global_mask.transpose(1, 0)
        ).permute(
            0, 2, 1
        )  # ([B,HW,C] -> [B,C,HW]) x ([Nc,HW] -> [HW,Nc]) -> [B,C,Nc] -> [B,Nc,C]
        ...
        score = torch.bmm(global_queries, local_keys.transpose(1, 2)) / math.sqrt(
            global_queries.shape[-1]
        )  # [B,Nc,C] x ([B,HW,C] -> [B,C,HW]) -> [B,Nc,HW]
        attn = self.masked_softmax(
            vec=score, mask=self.local_mask.unsqueeze(0), dim=-1
        )  # [B,Nc,HW]
    ...
    def self_attention_cpg(self, batch):
        ...
        output = torch.einsum(
            "abcd,abcde->abce", sa_encodings, cpg_matrix
        )  # [B,HW,D] x [B,HW,D,C] -> [B,HW,C]
        return output
    ...
    def forward(self, batch):
        ...
        batch = self.input_padder(
            batch
        )  # Pad batch for resolution reduction/preservation
    ...
    def flops(self, N=None):
        ...
        if "Three_unmasked_base" == self.approach_name:
            ...
            # Similarity Creation
            window_flops += (
                N * self.spatial_W * self.spatial_H * self.spatial_C * self.spatial_C
            )
            # Softmax
            window_flops += N * self.spatial_W * self.spatial_H * 2
            # Filter Creation
            window_flops += (
                N * self.spatial_H * self.spatial_H * self.spatial_H * self.spatial_W
            )
            # Sum Projections
...
class ConvAttnWrapper(nn.Module):
    def __init__(self, backbone, variant_kwargs):
        ...
        (
            ...
            self.backbone_spatial_shapes,
        ) = self.backbone.get_network_structure()
