...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        self.reverse_activation = self.reverse_activation_fn(
            mechanism_instructions.get("reverse_activation", "none")
        )
    ...
    def instantiate_generator_weights(self):
        if "reverse" in self.mechanism:
            ...
            self.weight_generator = nn.Linear(
                self.reverse_dim, self.embed_dim * self.embed_dim, bias=False
            )
            ...
            if self.reduce_reverse and self.mechanism_instructions.get(
                "project_input", True
            ):
                # pdb.set_trace()
    ...
    def apply_forward_attention(self, x, attn):
        ...
        v = (
            self.forward_v(x)
            .reshape(BW, K2, self.num_heads, C // self.num_heads)
            .transpose(2, 1)
        )
    ...
    def apply_reverse_attention(self, x, attn):
        ...
        if self.reduce_reverse:
            if self.mechanism_instructions.get("gen_indiv_hyper_weights", False):
                # pdb.set_trace()
                v_weights = self.weight_generator(v).reshape(
                    BW, self.num_heads, K2, self.embed_dim, self.embed_dim
                )
                if self.mechanism_instructions.get("activate_hyper_weights", False):
                    v_weights = self.reverse_activation(v_weights)
                weights = (attn @ v_weights.flatten(-2)).reshape(
                    BW, self.num_heads, K2, self.embed_dim, self.embed_dim
                )
            else:
                weights = self.weight_generator(expert_mixture).reshape(
                    BW, self.num_heads, K2, self.embed_dim, self.embed_dim
                )
                # pdb.set_trace()
                if self.mechanism_instructions.get("activate_hyper_weights", False):
            ...
            proj_weight = self.weight_generator(x).reshape(
        ...
        if self.hypernetwork_bias:
            proj_bias = (
                self.bias_generator(expert_mixture).transpose(2, 1).reshape(BW, K2, C)
            )
    ...
    def forward(self, x, mask=None):
        ...
        qk = (
            self.qk(x)
            .reshape(BW, K2, 2, self.num_heads, C // self.num_heads)
            .permute(2, 0, 3, 1, 4)
        )
